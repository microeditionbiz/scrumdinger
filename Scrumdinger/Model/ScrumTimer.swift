...
extension Array where Element == ScrumTimer.Speaker {
    var speakerNumber: Int? {
        guard let index = firstIndex(where: { !$0.isCompleted }) else { return nil }
        return index + 1
    }

    var isLastSpeaker: Bool {
        return dropLast().allSatisfy { $0.isCompleted }
    }
}
